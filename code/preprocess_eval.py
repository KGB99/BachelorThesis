...
def eval_yolact(args):
    ...
    for i,camera in enumerate(test_annotations_dict):
        ...
        for j, image in enumerate(test_annotations_dict[camera]):
            ...
            eval_dict['image_id'] = gt_dict['img']['id']
            eval_dict['image_file'] = gt_dict['img']['file_name']

            if gt_dict['gt_exists'] == 0:
                eval_dict['gt_exists'] = 0
            else:
                eval_dict['gt_exists'] = 1
                eval_dict['gt_cat_id'] = gt_dict['mask']['category_id']
                gt_bbox = gt_dict['mask']['bbox']
                gt_seg_vertices = gt_dict['mask']['segmentation']
                gt_img_id = gt_dict['mask']['image_id']
                gt_cat_id = gt_dict['mask']['category_id']
            ...
            try:
                ...
                continue
            #add to eval_dict that a prediction was found
            eval_dict['pred_exists'] = 1
            
            if eval_dict['gt_exists'] == 0:
                print(" No prediction and ground truth!")
                continue
            ...
            eval_dict['result_img_file'] = (output_images_path + '/' + str(gt_dict['img']['id']) + '_' \
                                            + str(gt_dict['mask']['category_id']) + '.jpg')
            ...
            if VIDEO:
                ...
                cv2.imwrite(output_images_path + '/' + str(gt_dict['img']['id']) + '_' 
                            + str(gt_dict['mask']['category_id']) + '.jpg', result)
